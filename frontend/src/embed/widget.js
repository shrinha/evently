...
(function(scriptElement) {
    ...
    const loadWidget = () => {
        ...
        const widgets = document.querySelectorAll('.evently-widget');
        widgets.forEach((widget, index) => {
            const eventId = widget.getAttribute('data-evently-id');
            if (!eventId) {
                console.error('HiEvent widget error: data-evently-id is required');
                return;
            }
            ...
            iframe.setAttribute('title', 'Evently Widget');
            iframe.style.border = 'none';
            iframe.style.width = '100%';

            const iframeId = `evently-iframe-${index}`;
            iframe.id = iframeId;

            let src = `${scriptOrigin}/widget/${encodeURIComponent(eventId)}?iframeId=${iframeId}&`;
            const params = [];
            Array.from(widget.attributes).forEach(attr => {
                if (attr.name.startsWith('data-evently-') && attr.name !== 'data-evently-id') {
            ...
            const autoResize = widget.getAttribute('data-evently-autoresize') !== 'false';
