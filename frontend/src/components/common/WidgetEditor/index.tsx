...
export const WidgetEditor = () => {
    ...
    useEffect(() => {
        setHtmlEmbedCode(
            '<div ' +
            'data-evently-id="' + eventId + '" ' +
            'data-evently-primary-color="' + form.values.primary_color + '" ' +
            'data-evently-primary-text-color="' + form.values.primary_text_color + '" ' +
            'data-evently-secondary-color="' + form.values.secondary_color + '" ' +
            'data-evently-secondary-text-color="' + form.values.secondary_text_color + '" ' +
            'data-evently-background-color="' + form.values.background_color + '" ' +
            'data-evently-widget-type="widget" ' +
            'data-evently-widget-version="1.0" ' +
            'data-evently-locale="en" ' +
            'data-evently-padding="' + form.values.padding + 'px" ' +
            'data-evently-autoresize="' + form.values.autoResize + '" ' +
            'data-evently-continue-button-text="' + form.values.continue_button_text + '" ' +
            'class="evently-widget">' +
            '</div>'
        );
...
const EventlyWidget = ({
  ...
  return (
    <div
      data-evently-id={eventId}
      data-evently-primary-color={primaryColor}
      data-evently-primary-text-color={primaryTextColor}
      data-evently-secondary-color={secondaryColor}
      data-evently-secondary-text-color={secondaryTextColor}
      data-evently-background-color={backgroundColor}
      data-evently-widget-type={widgetType}
      data-evently-widget-version={widgetVersion}
      data-evently-locale={locale}
      className="evently-widget"
    />
  );
};

export default EventlyWidget;
        `);

        setReactUsageCode(`
import React from 'react';
import EventlyWidget from './EventlyWidget';
